...
def disc_pre_train():
    ...
    h_disc.hier_train_step(disc_config, evl_config)
...
def disc_train_data(sess, gen_model, vocab, source_inputs, source_outputs,
    ...
    for query, answer in zip(source_inputs, source_outputs):
        query = query[:query_len] + [data_utils.PAD_ID] * (query_len - len(query) if query_len > len(query) else 0)
        train_query.append(query)
        answer = answer[:answer_len] + [data_utils.PAD_ID] * (answer_len - len(answer) if answer_len > len(answer) else 0)
...
# discriminator api
def disc_step(sess, disc_model, train_query, train_answer, train_labels, forward_only=False):
    feed_dict={}
    ...
    if forward_only:
        fetches = [disc_model.accuracy,disc_model.logits]
        accuracy, logits = sess.run(fetches, feed_dict)
    else:
        fetches = [disc_model.train_op, disc_model.cost, disc_model.accuracy, disc_model.logits]
        train_op, cost, accuracy, logits = sess.run(fetches,feed_dict)
    ...
    print("the train accuracy is: %f and the train accuracy is %f ."%(accuracy, reward))
    return reward
...
def al_train():
    ...
    with tf.Session() as sess:
        ...
        while True:
            ...
            encoder_inputs, decoder_inputs, target_weights, source_inputs, source_outputs = gen_model.get_batch(train_set, bucket_id, 0)
            # source_inputs = []
            # source_outputs = []
            # for i in xrange(disc_config.batch_size):
            #     source_input, source_output = random.choice(train_set[bucket_id])
            ...
            disc_step(sess, disc_model, train_query, train_answer, train_labels, forward_only=False)
            ...
            reward = disc_step(sess, disc_model, train_query, train_answer, train_labels, forward_only=True)
            ...
            if current_step % steps_per_checkpoint:
                ...
                disc_model.saver.save(sess, disc_model_path, global_step=disc_model.globle_step)
                ...
                gen_model.saver.save(sess, gen_model_path, global_step=gen_model.globle_step)
